...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".xbot" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".xbot" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".xbot" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".xbot" / "workspace"
