...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("xbot.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "xbot.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import xbot.channels.telegram as tg_mod
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "xbot.channels.telegram.get_media_dir",
