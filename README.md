...
**1. Ask xbot to set up Mochat for you**

Simply send this message to xbot (replace `xxx@xxx` with your real email):

```
Read https://raw.githubusercontent.com/HKUDS/MoChat/refs/heads/main/skills/xbot/skill.md and register on MoChat. My Email account is xxx@xxx Bind me as your owner and DM me on MoChat.
```

xbot will automatically register, configure `~/.xbot/config.json`, and connect to Mochat.
...
That's it — xbot handles the rest!
...
If you prefer to configure manually, add the following to `~/.xbot/config.json`:
...
- You need:
  - `userId` (example: `@xbot:matrix.org`)
  - `accessToken`
...
> After upgrading xbot, rebuild the local bridge with:
> `rm -rf ~/.xbot/bridge && xbot channels login`
...
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running xbot first to establish connection)
...
> `allowFrom`: Add your open_id (find it in xbot logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in xbot logs when you message the bot). Use `["*"]` for public access.
...
Give xbot its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-xbot@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "my-xbot@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "my-xbot@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "my-xbot@gmail.com",
...
🐈 xbot is capable of linking to the agent social network (agent community). **Just send one message and your xbot joins automatically!**
...
Simply send the command above to your xbot (via CLI or any chat channel), and it will handle the rest.
...
xbot supports multiple web search providers. Configure in `~/.xbot/config.json` under `tools.web.search`.
...
When credentials are missing, xbot automatically falls back to DuckDuckGo.
...
xbot supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped xbot tool name (for example `mcp_filesystem_write_file`).
...
xbot gateway --config ~/.xbot-discord/config.json

# Instance C - Feishu bot with custom port
xbot gateway --config ~/.xbot-feishu/config.json --port 18792
```

### Path Resolution

When using `--config`, xbot derives its runtime data directory from the config file location. The workspace still comes from `agents.defaults.workspace` unless you override it with `--workspace`.

To open a CLI session against one of these instances locally:

```bash
xbot agent -c ~/.xbot-telegram/config.json -m "Hello from Telegram instance"
xbot agent -c ~/.xbot-discord/config.json -m "Hello from Discord instance"

# Optional one-off workspace override
xbot agent -c ~/.xbot-telegram/config.json -w /tmp/xbot-telegram-test
```

> `xbot agent` starts a local CLI agent using the selected workspace/config. It does not attach to or proxy through an already running `xbot gateway` process.

| Component | Resolved From | Example |
|-----------|---------------|---------|
| **Config** | `--config` path | `~/.xbot-A/config.json` |
| **Workspace** | `--workspace` or config | `~/.xbot-A/workspace/` |
| **Cron Jobs** | config directory | `~/.xbot-A/cron/` |
| **Media / runtime state** | config directory | `~/.xbot-A/media/` |
...
```bash
xbot gateway --config ~/.xbot-telegram/config.json --workspace /tmp/xbot-telegram-test
```
...
| `xbot agent` | Interactive chat mode |
| `xbot agent --no-markdown` | Show plain-text replies |
...
> **Note:** The gateway must be running (`xbot gateway`) and you must have chatted with the bot at least once so it knows which channel to deliver to.
...
> [!TIP]
> The `-v ~/.xbot:/root/.xbot` flag mounts your local config directory into the container, so your config and workspace persist across container restarts.
...
```bash
docker compose run --rm xbot-cli agent -m "Hello!"   # run CLI
docker compose logs -f xbot-gateway                   # view logs
...
docker run -v ~/.xbot:/root/.xbot -p 18790:18790 xbot gateway

# Or run a single command
docker run -v ~/.xbot:/root/.xbot --rm xbot agent -m "Hello!"
docker run -v ~/.xbot:/root/.xbot --rm xbot status
```
...
**1. Find the xbot binary path:**

```bash
which xbot   # e.g. /home/user/.local/bin/xbot
```

**2. Create the service file** at `~/.config/systemd/user/xbot-gateway.service` (replace `ExecStart` path if needed):
...
[Service]
Type=simple
ExecStart=%h/.local/bin/xbot gateway
Restart=always
RestartSec=10
NoNewPrivileges=yes
...
systemctl --user enable --now xbot-gateway
```

**Common operations:**

```bash
systemctl --user status xbot-gateway        # check status
systemctl --user restart xbot-gateway       # restart after config changes
journalctl --user -u xbot-gateway -f        # follow logs
```
