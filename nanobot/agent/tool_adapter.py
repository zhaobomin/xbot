"""Tool adapter for converting nanobot tools to MCP format.

This module adapts nanobot's Tool implementations to MCP tools
...
from nanobot.agent.capabilities import canonical_tool_name
...
class ToolAdapter:
    """Adapts nanobot tools to MCP format.

    This class:
    - Creates MCP versions of nanobot's built-in tools
    ...
    def create_mcp_server(self) -> dict[str, Any]:
        """Create an MCP server with all nanobot tools.
        ...
        # Register nanobot-specific tools
        if not self._tools:
    ...
    def _adapt_tool(self, tool_name: str, tool_instance: Any) -> Any:
        """Adapt a nanobot Tool to MCP format.
