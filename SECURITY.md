...
If you discover a security vulnerability in xbot, please report it by:
...
chmod 600 ~/.xbot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.xbot/config.json` with file permissions set to `0600`
...
- ✅ Run xbot with a dedicated user account
...
- Keep authentication data in `~/.xbot/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for xbot and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.xbot
   chmod 600 ~/.xbot/config.json
   chmod 700 ~/.xbot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.xbot/logs/xbot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.xbot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.xbot/logs/xbot.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/xbot/security/advisories
- Release Notes: https://github.com/HKUDS/xbot/releases
